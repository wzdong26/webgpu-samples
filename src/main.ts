...
const _inputPanel = createEl({ className: 'input-panel' });
const _canvasContainer = createEl({ className: 'canvas-container', children: [_canvas, _inputPanel] });
...
const loadPage = (url?: string) => {
    ...
    pages[key as keyof typeof pages]?.().then(async ({ render }) => {
        pause = await render?.(_canvas, _inputPanel);
    });
};
